// magneto 1.4 magnetometer/accelerometer calibration code
// from http://sailboatinstruments.blogspot.com/2011/08/improved-magnetometer-calibration.html

#include <math.h>
#include <malloc.h>

#include "mymathlib_matrix.h"

#include "magneto1_4.h"

double *C, *S11, *S12, *S12t, *S22, *S22a, *S22b, *SS, *E, *U, *SSS;
double *eigen_real, *eigen_imag, *v1, *v2, *v, *Q, *Q_1, *B, *QB, *SSSS;
double *eigen_real3, *eigen_imag3, *Dz, *vdz, *SQ, *A_1;

void magneto_sample(double x, double y, double z, double *ata, double *norm_sum, double *sample_count)
{
    *sample_count += 1.0;
    *norm_sum += sqrt(x * x + y * y + z * z);

    double D[10] = {
        x * x,
        y * y,
        z * z,
        2.0 * y * z,
        2.0 * x * z,
        2.0 * x * y,
        2.0 * x,
        2.0 * y,
        2.0 * z,
        1.0};

    Multiply_Self_Transpose(ata, D, 10, 1);
}

void magneto_current_calibration(float BAinv[4][3], double *ata, double norm_sum, double sample_count)
{
    S11 = (double *)calloc(6 * 6, sizeof(double));
    Get_Submatrix(S11, 6, 6, ata, 10, 0, 0);
    S12 = (double *)calloc(6 * 4, sizeof(double));
    Get_Submatrix(S12, 6, 4, ata, 10, 0, 6);
    S12t = (double *)calloc(4 * 6, sizeof(double));
    Get_Submatrix(S12t, 4, 6, ata, 10, 6, 0);
    S22 = (double *)calloc(4 * 4, sizeof(double));
    Get_Submatrix(S22, 4, 4, ata, 10, 6, 6);

    double hm = norm_sum / sample_count;

    Choleski_LU_Decomposition(S22, 4);
    Choleski_LU_Inverse(S22, 4);

    S22a = (double *)calloc(4 * 6, sizeof(double));
    Multiply_Matrices(S22a, S22, 4, 4, S12t, 6);
    free(S22);
    free(S12t);

    S22b = (double *)calloc(6 * 6, sizeof(double));
    Multiply_Matrices(S22b, S12, 6, 4, S22a, 6);
    free(S12);

    SS = (double *)calloc(6 * 6, sizeof(double));
    for (int i = 0; i < 36; i++)
        SS[i] = S11[i] - S22b[i];
    free(S11);
    free(S22b);

    C = (double *)calloc(6 * 6, sizeof(double));
    C[0] = 0.0;
    C[1] = 0.5;
    C[2] = 0.5;
    C[3] = 0.0;
    C[4] = 0.0;
    C[5] = 0.0;
    C[6] = 0.5;
    C[7] = 0.0;
    C[8] = 0.5;
    C[9] = 0.0;
    C[10] = 0.0;
    C[11] = 0.0;
    C[12] = 0.5;
    C[13] = 0.5;
    C[14] = 0.0;
    C[15] = 0.0;
    C[16] = 0.0;
    C[17] = 0.0;
    C[18] = 0.0;
    C[19] = 0.0;
    C[20] = 0.0;
    C[21] = -0.25;
    C[22] = 0.0;
    C[23] = 0.0;
    C[24] = 0.0;
    C[25] = 0.0;
    C[26] = 0.0;
    C[27] = 0.0;
    C[28] = -0.25;
    C[29] = 0.0;
    C[30] = 0.0;
    C[31] = 0.0;
    C[32] = 0.0;
    C[33] = 0.0;
    C[34] = 0.0;
    C[35] = -0.25;
    E = (double *)calloc(6 * 6, sizeof(double));
    Multiply_Matrices(E, C, 6, 6, SS, 6);
    free(C);
    free(SS);

    SSS = (double *)calloc(6 * 6, sizeof(double));
    Hessenberg_Form_Elementary(E, SSS, 6);

    int index = 0;
    {
        eigen_real = (double *)calloc(6, sizeof(double));
        eigen_imag = (double *)calloc(6, sizeof(double));

        QR_Hessenberg_Matrix(E, SSS, eigen_real, eigen_imag, 6, 100);
        free(E);

        double maxval = eigen_real[0];
        for (int i = 1; i < 6; i++)
        {
            if (eigen_real[i] > maxval)
            {
                maxval = eigen_real[i];
                index = i;
            }
        }
        free(eigen_real);
        free(eigen_imag);
    }

    v1 = (double *)calloc(6, sizeof(double));
    v1[0] = SSS[index];
    v1[1] = SSS[index + 6];
    v1[2] = SSS[index + 12];
    v1[3] = SSS[index + 18];
    v1[4] = SSS[index + 24];
    v1[5] = SSS[index + 30];
    free(SSS);

    double norm = sqrt(v1[0] * v1[0] + v1[1] * v1[1] + v1[2] * v1[2] + v1[3] * v1[3] + v1[4] * v1[4] + v1[5] * v1[5]);
    for (int i = 0; i < 6; i++)
        v1[i] /= norm;

    if (v1[0] < 0.0)
        for (int i = 0; i < 6; i++)
            v1[i] = -v1[i];

    v2 = (double *)calloc(4, sizeof(double));
    Multiply_Matrices(v2, S22a, 4, 6, v1, 1);
    free(S22a);

    U = (double *)calloc(3, sizeof(double));
    Q = (double *)calloc(3 * 3, sizeof(double));
    double J;

    v = (double *)calloc(10, sizeof(double));
    for (int i = 0; i < 6; i++)
        v[i] = v1[i];
    free(v1);
    for (int i = 0; i < 4; i++)
        v[6 + i] = -v2[i];
    free(v2);

    Q[0] = v[0];
    Q[1] = v[5];
    Q[2] = v[4];
    Q[3] = v[5];
    Q[4] = v[1];
    Q[5] = v[3];
    Q[6] = v[4];
    Q[7] = v[3];
    Q[8] = v[2];

    U[0] = v[6];
    U[1] = v[7];
    U[2] = v[8];

    J = v[9];
    free(v);

    B = (double *)calloc(3, sizeof(double));
    Q_1 = (double *)calloc(3 * 3, sizeof(double));
    for (int i = 0; i < 9; i++)
        Q_1[i] = Q[i];
    Choleski_LU_Decomposition(Q_1, 3);
    Choleski_LU_Inverse(Q_1, 3);
    Multiply_Matrices(B, Q_1, 3, 3, U, 1);
    free(U);
    free(Q_1);
    for (int i = 0; i < 3; i++)
        B[i] = -B[i];

    double btqb;
    QB = (double *)calloc(3, sizeof(double));
    Multiply_Matrices(QB, Q, 3, 3, B, 1);
    Multiply_Matrices(&btqb, B, 1, 3, QB, 1);
    free(QB);

    SSSS = (double *)calloc(3 * 3, sizeof(double));
    Hessenberg_Form_Elementary(Q, SSSS, 3);
    Dz = (double *)calloc(3 * 3, sizeof(double));
    eigen_real3 = (double *)calloc(3, sizeof(double));
    eigen_imag3 = (double *)calloc(3, sizeof(double));
    QR_Hessenberg_Matrix(Q, SSSS, eigen_real3, eigen_imag3, 3, 100);
    free(Q);

    Dz[0] = sqrt(eigen_real3[0]);
    Dz[4] = sqrt(eigen_real3[1]);
    Dz[8] = sqrt(eigen_real3[2]);
    free(eigen_real3);
    free(eigen_imag3);

    {
        double norm = sqrt(SSSS[0] * SSSS[0] + SSSS[3] * SSSS[3] + SSSS[6] * SSSS[6]);
        SSSS[0] /= norm;
        SSSS[3] /= norm;
        SSSS[6] /= norm;
        norm = sqrt(SSSS[1] * SSSS[1] + SSSS[4] * SSSS[4] + SSSS[7] * SSSS[7]);
        SSSS[1] /= norm;
        SSSS[4] /= norm;
        SSSS[7] /= norm;
        norm = sqrt(SSSS[2] * SSSS[2] + SSSS[5] * SSSS[5] + SSSS[8] * SSSS[8]);
        SSSS[2] /= norm;
        SSSS[5] /= norm;
        SSSS[8] /= norm;
    }

    SQ = (double *)calloc(3 * 3, sizeof(double));
    vdz = (double *)calloc(3 * 3, sizeof(double));
    Multiply_Matrices(vdz, SSSS, 3, 3, Dz, 3);
    free(Dz);
    Transpose_Square_Matrix(SSSS, 3);
    Multiply_Matrices(SQ, vdz, 3, 3, SSSS, 3);
    free(SSSS);
    free(vdz);

    A_1 = (double *)calloc(3 * 3, sizeof(double));

    printk("btqb: %f, J: %f, btqb - J: %f\n", btqb, J, btqb - J);
    double hmb_candidate = btqb - J;
    if (hmb_candidate < 0) {
        printf("Error: btqb - J is negative: %f\n", hmb_candidate);
        hmb_candidate = 1e-6; // 음수 값 방지
    }
    double hmb = sqrt(hmb_candidate);

    for (int i = 0; i < 9; i++)
        A_1[i] = SQ[i] * hm / hmb;
    free(SQ);

    for (int i = 0; i < 3; i++)
        BAinv[0][i] = B[i];
    free(B);

    for (int i = 0; i < 3; i++) {
        BAinv[i + 1][0] = A_1[i * 3];
        BAinv[i + 1][1] = A_1[i * 3 + 1];
        BAinv[i + 1][2] = A_1[i * 3 + 2];
    }
    free(A_1);
}
